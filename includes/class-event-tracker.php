...
class Event_Tracker {
    ...
    public function __construct() {
        ...
        if ( get_option( MARKETPULSE_OPTION_FEATURE_EVENT_TRACKING, '1' ) !== '1' ) {
            return;
        }
    ...
    public function flush_events() {
        ...
        $this->api_client->make_request( MARKETPULSE_ENDPOINT_EVENTS_INGEST, array(
    ...
    public function inject_tracker_script() {
        ...
        echo '<script>window.__MP={k:"' . esc_js( $license_key ) . '",api:"' . esc_js( MARKETPULSE_API_BASE_URL ) . '/events/ingest"};</script>' . "\n";
        echo '<script src="' . esc_url( MARKETPULSE_PLUGIN_URL . 'assets/js/tracker.js' ) . '?v=' . MARKETPULSE_VERSION . '" defer></script>' . "\n";
    }
